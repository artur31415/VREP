...
if (simGetScriptExecutionCount()==0) then
        ...
        Serial = simSerialOpen('COM1',9600)
        filePath = "C:\\Users\\sigma\\Documents\\PROJECTS!!\\kalman filter\\Data\\TruePos.txt"
        Limitline = "C:\\Users\\sigma\\Documents\\PROJECTS!!\\kalman filter\\Data\\Encoder.txt"
        Sensorsline = "C:\\Users\\sigma\\Documents\\PROJECTS!!\\kalman filter\\Data\\IMU.txt"
    ...
    --number result,number distance,table_3 detectedPoint,number detectedObjectHandle,table_3 detectedSurfaceNormalVector=simReadProximitySensor(number sensorHandle)
    
        UltraSonico = simGetObjectHandle('Sonar0')
        SonarServo = simGetObjectHandle('SonarServo')
        ...
        ToRotate = false
        d = 0.0581 -- distance between wheel divided by 2
        ...
        ActualTurns = 0
        LastTurns = 0
        AxialDistance = {0.0, 0.0, 0.0, 0.0}
        IsSlamStart = true
    
        ServoDirection = 'CCW'
        ServoAngle = 0
        simSetJointTargetVelocity(SonarServo,0.5)
        MeasurementAngle = 0
    ...
    TruePos_data:write(simGetSimulationTime()..' '..pos[1]..' '..pos[2]..' '..pos[3]..' '..euler[1]..' '..euler[2]..' '..euler[3]..'\n')
        if(simGetSimulationTime() - SerialLastTime >= 0.1) then
        SerialLastTime = simGetSimulationTime()
        local an = euler[2]*RAD2DEG
            
        --simAuxiliaryConsolePrint(Console,'angule = '..string.format('%.3f',an)..'\n----------------------------------------\n') MyPosition[4] MeasurementAngle
            if(Serial ~= -1) then
            simSerialSend(Serial,string.format('%.3f',pos[1])..' '..string.format('%.3f',pos[2])..' '..string.format('%.3f',MeasurementAngle)..' '..string.format('%.3f',Dist)..'\n') --Dist
            end
        end
    end
    ...
    function SLAM_Locomotion()
        if(NumOfTurns >= 1) then
        IsSlamStart = false
        ToRotate = false
        end
        ...
        if(IsSlamStart) then
        ToRotate = true
        elseif(Dist <= 0.25 and Dist ~= -1 and not ToRotate) then -- Turn 360 deg then go in the first clear direction
        ToRotate = true
        LastTurns = NumOfTurns
        
        end
        
        if(ToRotate) then
        Right_axis()
            if( not IsSlamStart and NumOfTurns - LastTurns >= 1)then
                if(AxialDistance[1] == -1 and MyPosition[4] <= 5) then --go in this direction
                ToRotate = false
                elseif(AxialDistance[2] == -1 and MyPosition[4] <= 95) then --go in this direction
                ToRotate = false
                elseif(AxialDistance[3] == -1 and MyPosition[4] <= 185) then --go in this direction
                ToRotate = false
                elseif(AxialDistance[4] == -1 and MyPosition[4] <= 275) then --go in this direction
                ToRotate = false
                end
            end
        else
    ...
    function Servo()
    if(simGetJointPosition(SonarServo)*RAD2DEG < 0 and simGetJointPosition(SonarServo)*RAD2DEG >= -180) then
    ServoAngle = - (180 + simGetJointPosition(SonarServo)*RAD2DEG)
    elseif(simGetJointPosition(SonarServo)*RAD2DEG > 0 and simGetJointPosition(SonarServo)*RAD2DEG <= 180) then
    ServoAngle = 180 - simGetJointPosition(SonarServo)*RAD2DEG
    end
    
    if(ToRotate) then
        if(not(ServoAngle > -5 and ServoAngle < 5)) then
            if(simGetJointPosition(SonarServo)*RAD2DEG > 0 and simGetJointPosition(SonarServo)*RAD2DEG <= 135 and ServoDirection == 'CW') then
            simSetJointTargetVelocity(SonarServo,0.5)
            ServoDirection = 'CCW'
            elseif(simGetJointPosition(SonarServo)*RAD2DEG < 0 and simGetJointPosition(SonarServo)*RAD2DEG >= -135 and ServoDirection == 'CCW') then
                simSetJointTargetVelocity(SonarServo,-0.5)
                ServoDirection = 'CW'
            end
        else
            simSetJointTargetVelocity(SonarServo,0)
            ServoDirection = 'S'
        end
    else
        if(ServoDirection == 'S') then
            simSetJointTargetVelocity(SonarServo,0.5)
            ServoDirection = 'CCW'
        elseif(simGetJointPosition(SonarServo)*RAD2DEG > 0 and simGetJointPosition(SonarServo)*RAD2DEG <= 135 and ServoDirection == 'CW') then
            simSetJointTargetVelocity(SonarServo,0.5)
            ServoDirection = 'CCW'
        elseif(simGetJointPosition(SonarServo)*RAD2DEG < 0 and simGetJointPosition(SonarServo)*RAD2DEG >= -135 and ServoDirection == 'CCW') then
            simSetJointTargetVelocity(SonarServo,-0.5)
    ...
    MeasurementAngle = ServoAngle + MyPosition[4]
    if(MeasurementAngle < 0) then
    MeasurementAngle = 360 + MeasurementAngle
    end
    ...
    --result,Dist,detectedPoint,detectedObjectHandle,detectedSurfaceNormalVector=simReadProximitySensor(sonar)--UltraSonico
    --result,Dist=simReadProximitySensor(sonar)
    result1,Dist=simReadProximitySensor(UltraSonico)--UltraSonico
    Dist = Dist or -1
    --dist = dist or -1
    if(MeasurementAngle >= 0 and MeasurementAngle <= 5) then
    AxialDistance[1] = Dist
    elseif(MeasurementAngle >= 85 and MeasurementAngle <= 95) then
    AxialDistance[2] = Dist
    elseif(MeasurementAngle >= 175 and MeasurementAngle <= 195) then
    AxialDistance[3] = Dist
    elseif(MeasurementAngle >= 265 and MeasurementAngle <= 275) then
    AxialDistance[4] = Dist
    end
    --simSetJointTargetPosition ServoAngle
    
    simAuxiliaryConsolePrint(Console,string.format('AxialDistance[1] = %.4f',AxialDistance[1])..'\n'..string.format('AxialDistance[2] = %.4f',AxialDistance[2])..'\n'..string.format('AxialDistance[3] = %.4f',AxialDistance[3])..'\n'..string.format('AxialDistance[4] = %.4f',AxialDistance[4])..'\n')
    simAuxiliaryConsolePrint(Console,'simGetJointPosition = '..(simGetJointPosition(SonarServo)*RAD2DEG)..'\nservoAngle = '..(ServoAngle)..'\nMeasurementAngle = '..MeasurementAngle..'\n----------------------------------------\n')
    SLAM_Locomotion()
    --Forward()
    ...
    --simAuxiliaryConsolePrint(Console,'result = '..result..'\n'..'distance = '..Dist..'\n----------------------------------------\n')
